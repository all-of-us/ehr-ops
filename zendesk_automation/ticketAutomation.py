...
def comment_update(zenpy_client, search, comment_body):
...
def create_ticket(zenpy_client, submission_tracking_df, src_hpo_id, site_contact_df, scores, metric, table_name, tag_list):
    ...
    requester = zenpy_client.search(type='user', email='em3697@cumc.columbia.edu')
    requester_id = list(requester)[0].id
    ...
    ticket_audit = zenpy_client.tickets.create(Ticket(requester_id=requester_id, assignee_id=assignee_id, 
...
def get_hpo_list(filled_scores_df, metric, threshold):
    hpo_list = filled_scores_df[(filled_scores_df[f'condition_{metric}'] < threshold) | 
...
def get_table_metric(row, metric):
    metric_by_table = {f'_{metric}': row[f'_{metric}'], 
...
def evaluate_metrics(zenpy_client, scores, metric, src_hpo_id, submission_tracking_df, site_contact_df):
    for name, score in scores.items():
        ...
        if score < 0.9:
            ...
            if len(search) > 0:
                comment_update(zenpy_client, search, COMMENT_BODY)

            # If the search does not return a ticket then create a new ticket
            else:
                create_ticket(zenpy_client, submission_tracking_df, src_hpo_id, site_contact_df, scores, metric, table_name, tag_list)
...
def ticket_automation():
    ...
    for metric in metrics:
        ...
        if filled_scores_df.shape[0] > 0:
            ...
            low_scores_df = get_hpo_list(filled_scores_df, metric, threshold = 0.9)
            ...
            for _ , row in low_scores_df.iterrows():
                ...
                evaluate_metrics(zenpy_client, scores, metric, src_hpo_id, submission_tracking_df, site_contact_df)
