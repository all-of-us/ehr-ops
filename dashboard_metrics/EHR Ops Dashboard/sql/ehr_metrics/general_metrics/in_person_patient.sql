...
AND a.consent_value = 'ConsentPermission_Yes' AND a.consent_expired is null),
...
count(distinct case when ipp.participant_id is null and psy.participant_id is not null then psy.participant_id end) as patient_only,
count(distinct case when ipp.participant_id is not null and psy.participant_id is null then ipp.participant_id end) as in_person_only
FROM in_person_participant ipp
FULL OUTER JOIN patient_status_yes psy on ipp.participant_id = psy.participant_id and ipp.external_id = psy.external_id
WHERE ipp.participant_id is not null
