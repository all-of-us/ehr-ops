...
with
    ...
    COALESCE(vd.wrong_start_date_rows, 0) as visit_start_wrong_date_rows,
    COALESCE(vd.total_end_rows, 0) as visit_end_total_rows,
    COALESCE(vd.wrong_end_date_rows, 0) as visit_end_wrong_date_rows,
    COALESCE(cd.total_start_rows, 0) as condition_start_total_rows,
    COALESCE(cd.wrong_start_date_rows, 0) as condition_start_wrong_date_rows,
    COALESCE(cd.total_end_rows, 0) as condition_end_total_rows,
    COALESCE(cd.wrong_end_date_rows, 0) as condition_end_wrong_date_rows,
    COALESCE(dd.total_start_rows, 0) as drug_start_total_rows,
    COALESCE(dd.wrong_start_date_rows, 0) as drug_start_wrong_date_rows,
    COALESCE(dd.total_end_rows, 0) as drug_end_total_rows,
    COALESCE(dd.wrong_end_date_rows, 0) as drug_end_wrong_date_rows,
    COALESCE(m.total_rows, 0) as measurement_total_rows,
    COALESCE(m.wrong_date_rows, 0) as measurement_wrong_date_rows,
    COALESCE(o.total_rows, 0) as observation_total_rows,
    COALESCE(o.wrong_date_rows, 0) as observation_wrong_date_rows,
    COALESCE(p.total_rows, 0) as procedure_total_rows,
    COALESCE(p.wrong_date_rows, 0) as procedure_wrong_date_rows,
    COALESCE(ded.total_start_rows, 0) as device_start_total_rows,
    COALESCE(ded.wrong_start_date_rows, 0) as device_start_wrong_date_rows,
    COALESCE(ded.total_end_rows, 0) as device_end_total_rows,
    COALESCE(ded.wrong_end_date_rows, 0) as device_end_wrong_date_rows
    from visit_date vd
